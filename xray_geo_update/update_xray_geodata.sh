...
curl -s -m 15 -L -R -H "Cache-Control: no-cache" -o /tmp/geoip.dat.sha256sum $GEOIP_CHECKSUM_URL
if [[ $? == 0 ]]
then
  curl -s -m 15 -L -R -H "Cache-Control: no-cache" -o /tmp/geoip.dat $GEOIP_URL
  cd /tmp
...
curl -s -m 15 -L -R -H "Cache-Control: no-cache" -o /tmp/geosite.dat.sha256sum $GEOSITE_CHECKSUM_URL
if [[ $? == 0 ]]
then
  curl -s -m 15 -L -R -H "Cache-Control: no-cache" -o /tmp/geosite.dat $GEOSITE_URL
  cd /tmp
