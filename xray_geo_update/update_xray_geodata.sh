...
wget -T 5 -O /tmp/geoip.dat.sha256sum $GEOIP_CHECKSUM_URL
if [[ $? == 0 ]]
then
  wget -T 5 -O /tmp/geoip.dat $GEOIP_URL
  cd /tmp
...
wget -T 5 -O /tmp/geosite.dat.sha256sum $GEOSITE_CHECKSUM_URL
if [[ $? == 0 ]]
then
  wget -T 5 -O /tmp/geosite.dat $GEOSITE_URL
  cd /tmp
